...
import { AssistantMessage, UserCreditProfile } from '../../types/barin-assistant';
...
interface BarinAssistantWorkspaceProps {
  onSignInClick?: () => void;
  onContactAdminClick?: () => void;
}

export const BarinAssistantWorkspace: React.FC<BarinAssistantWorkspaceProps> = ({
  onSignInClick,
  onContactAdminClick,
}) => {
  const initialMessage: AssistantMessage = {
    id: 'msg-welcome',
    sender: 'ASSISTANT',
    text: 'Hello. I am Barin Assistant. I can help you understand Philippine laws, court rulings, legal procedures, and other general legal matters. My responses provide legal information and are not a substitute for advice from a licensed lawyer.',
  ...
  const [hasConsented, setHasConsented] = useState<boolean>(() => {
    try {
      return localStorage.getItem('barin_assistant_consented') === 'true';
    } catch {
      return false;
    }
  });
  ...
  const handleConsentConfirmed = () => {
    ...
    try {
      localStorage.setItem('barin_assistant_consented', 'true');
    } catch (e) {
      ...
      {creditProfile.isExhausted && (
        <div className="border-t border-neutral-200 bg-neutral-50 p-4 text-xs space-y-3">
          <div className="flex items-start gap-2.5 text-neutral-800">
            ...
            <div>
              ...
              <p className="text-neutral-600 mt-0.5">
                You have reached your free Barin Assistant allowance. Contact the administrator to request a subscription or additional credits.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 pt-1">
            <button
              type="button"
              onClick={onContactAdminClick || (() => window.open('mailto:admin@demo-barinlaw.ph', '_blank'))}
          ...
          <textarea
            ...
            placeholder={
              creditProfile.isExhausted
                ...
                : 'Ask Barin Assistant about Philippine laws, Supreme Court rulings, or court procedures (Enter to send, Shift+Enter for new line)...'
            }
